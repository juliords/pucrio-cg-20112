...
Object* loadObject(char *buffer, Scene* scene, FILE *file)
{
	int material;
	...
	if( sscanf( buffer, "SPHERE %d %lf %lf %lf %lf\n", &material, &radius, &pos1.x,&pos1.y,&pos1.z ) == 5 ) 
	{
		...
		return objCreateSphere( material, pos1, radius );
	} 
	else if( sscanf( buffer, "TRIANGLE %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", &material, &pos1.x, &pos1.y, &pos1.z, &pos2.x, &pos2.y, &pos2.z, &pos3.x, &pos3.y, &pos3.z, &tex1.x, &tex1.y, &tex2.x, &tex2.y, &tex3.x, &tex3.y) == 16 ) 
	{
		...
		return objCreateTriangle( material, pos1, pos2, pos3, tex1, tex2, tex3 );
	}
	else if( sscanf( buffer, "BOX %d %lf %lf %lf %lf %lf %lf\n", &material, &pos1.x, &pos1.y, &pos1.z, &pos2.x, &pos2.y, &pos2.z ) == 7 ) 
	{
		...
		return objCreateBox( material, pos1, pos2 );
	} 
	else if( sscanf( buffer, "MESH %d %lf %lf %lf %lf %lf %lf %s\n", &material, &pos1.x, &pos1.y, &pos1.z, &pos2.x, &pos2.y, &pos2.z, buffer ) == 8 ) 
	{
		...
		return objCreateMesh( material, pos1, pos2, buffer );
	}

	sscanf(buffer, " %s", str);

	if(!strcmp(str, "UNION") || !strcmp(str, "INTERSECTION") || !strcmp(str, "DIFFERENCE"))
	{
		Object *obj1, *obj2;
//		printf("DEBUG: Entrei...\n");

		if(fscanf(file, " %[^\n]", buffer) == EOF) return NULL;
//		printf("DEBUG1: %s\n", buffer);
		if(!(obj1 = loadObject(buffer,scene,file))) return NULL;

		if(fscanf(file, " %[^\n]", buffer) == EOF) return NULL;
//		printf("DEBUG2: %s\n", buffer);
		if(!(obj2 = loadObject(buffer,scene,file))) return NULL;
		fscanf(file, "\n");

		return objCreateSet(str, obj1, obj2);
	}
...
//		printf("DEBUG: %s\n", buffer);
		...
		{
			Object *obj = loadObject(buffer, scene, file);

			if(obj) scene->objects[scene->objectCount++] = obj;
			else printf( "sceLoad: Ignorando comando:\n %s\n", buffer );
		}
	}
